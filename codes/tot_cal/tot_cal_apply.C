...
void tot_cal_apply(
	...
	const std::string &cal_param_file = "parameters.txt",
	...
	//load my parameters
	std::vector<std::vector<double>> cal_pars = par_load(cal_param_file);

	//for det0 and det1
	cal_pars.insert(cal_pars.begin(), std::vector<double>{0,0,0});
	cal_pars.insert(cal_pars.begin(), std::vector<double>{0,0,0});
		...
		for (auto ihit = 0; ihit < spirit.hime_nHits; ihit++) {
			...
			if (useWalkCorrection) {
				if (tot < totThresh) {
					...
					tof -= (walkParams[0] * std::exp(-walkParams[1] * tot) + walkParams[2]);

				} else {
					// tof -= (walkParams[0] * totThresh + walkParams[1]);
					// tof -= (walkParams[0] * totThresh * totThresh + walkParams[1] * totThresh + walkParams[2]);
					tof -= (walkParams[0] * std::exp(-walkParams[1] * totThresh) + walkParams[2]);
				}
			}
	...
	for (int i = 0; i < 72; i++) {
		...
		hTotVsTofBar_cal[i]->Scale(1. / spiritEntries);
...
std::vector<std::vector<double>> par_load(const std::string &text_file){
    ...
    while (std::getline(f_in, line)) {
        ...
        parameters.push_back({d.A, d.k, d.B, d.A_err, d.k_err, d.B_err});
    }
