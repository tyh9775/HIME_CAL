...
void shadowAna(
	...
	TH2D *hPosVsTofBar[72];
	for (int i = 0; i < 72; i++) {
		hPosVsTofBar[i] = new TH2D(Form("hPosVsTofBar%d", i), "", 2000, -100, 300, hitPatternBins, -1000, 1000);
	...
	for (auto iEvt = 0; iEvt < spiritEntries; iEvt++) {
		...
		for (auto ihit = 0; ihit < spirit.hime_nHits; ihit++) {
			...
			if (layerId == 0 || layerId == 2) {
				hPosVsTofBar[moduleId]->Fill(tof, xHit);
			} else {
				hPosVsTofBar[moduleId]->Fill(tof, yHit);
			}
